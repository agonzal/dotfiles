...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- Launch apps with keybindings
    , ((modm,               xK_f     ), spawn "firefox")
    , ((modm .|. controlMask, xK_r   ), spawn "calibre") -- Launch reading library
    -- lock screen
    , ((modm,               xK_F1    ), spawn "i3lock -e -u -t -i ~/Pictures/wallpapers/lovelace.png")

    -- launch rofi and dashboard
    , ((modm,               xK_o     ), spawn "~/bin/launcher.sh")
    , ((modm,               xK_p     ), spawn "~/bin/centerlaunch")
    , ((modm .|. shiftMask, xK_p     ), spawn "exec ~/bin/ewwclose")
    , ((modm .|. controlMask, xK_s   ), spawn "exec ~/bin/ssh.sh")
    ...
    -- Screenshoti / Visualizer
    , ((0,                    xK_Print), spawn "exec ~/bin/screenshot.sh")
...
myManageHook = fullscreenManageHook <+> manageDocks <+> composeAll
    ...
    , className =? "lightcord"      --> doShift ( myWorkspaces !! 5 )
...
myStartupHook = do
  ...
  spawnOnce "feh --bg-scale ~/Pictures/wallpapers/fedora.png"
  spawnOnce "/usr/local/bin/picom -f"
  spawnOnce "greenclip daemon"
  spawnOnce "dunst"
  spawnOnce "exec ~/.screenlayout/layout.sh"
