...
module FancyBorders (FancyBordersTheme(..), 
                     FancyBorders,
                     fancyBorders,
                     defaultFancyBorders,
                     defaultFancyTheme) where
...
defaultFancyTheme = FancyBordersTheme "#c8ccd4" 2 
...
data FancyBorders a = FancyBorders FancyBordersTheme
    deriving (Show, Read)

instance LayoutModifier FancyBorders Window where
    handleMess (FancyBorders cfg) m
        | Just (whatever :: Event) <- fromMessage m =
                withFocused (drawFocusedWindow cfg) >>
                return Nothing
        | otherwise = return Nothing
        
    redoLayout fb@(FancyBorders cfg) _ _ wrs = do
...
drawFancyBorder cfg (win, rect) active = do
    ...
    let w = rect_width rect - 2 * bw
        ...
        screen = 0
        rects = [Rectangle (fromIntegral w) 0 ibw (h + ibw),
                 ...
                 Rectangle (fromIntegral fw - fromIntegral ibw) (fromIntegral fh - fromIntegral ibw) ibw ibw]
